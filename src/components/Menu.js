...
const Menu = () =>{
   ...
   {/* {console.log(res?.cards[0]?.card?.card?.info)} */}
         ...
         <h2 className="text-4xl font-bold mx-12 " >{res?.cards[1]?.card?.card?.info?.name}</h2>
          <h3 className="text-xl ml-4 mt-3 ">{res?.cards[0]?.card?.card?.info?.cuisines.join(" , ")}</h3>
          <h3 className="text-xl"> {res?.cards[1]?.card?.card?.info?.areaName} , {res?.cards[1]?.card?.card?.info?.city}</h3>
          <h3 className="text-xl">{res?.cards[1]?.card?.card?.info?.avgRating} ⭐️
          </h3>
          <h3 className="text-xl">{res?.cards[1]?.card?.card?.info?.costForTwoMessage}</h3>
         </div>
               </div>
